...
module.exports = {
    ...
    command:function(builder, cb) {
        ...
        // build command:  ./cjs steal/buildjs OpsPortal opstools/MPDReport
        ...
        async.series([
            ...
            function(next){
                ...
                AD.log('<green>building</green> opstools/MPDReport');

                AD.spawn.command({
                    command:'./cjs',
                    // options:[path.join('opstools', 'MPDReport', 'build.js')],
                    options:[path.join('steal', 'buildjs'), 'OpsPortal', path.join('opstools', 'MPDReport')],
shouldEcho:true,
                    // exitTrigger:'opstools/MPDReport/production.css'
                })
                .fail(function(err){
                    AD.log.error('<red>could not complete opstools/MPDReport build!</red>');
                    next(err);
                })
                .then(function(){
            ...
            function(next) {
                ...
                var patches = [
                    { file:path.join('opstools', 'MPDReport', 'production.js'), tag:'packages/OpsPortal-MPDReport.js', replace:'OpsPortal/production.js'}
                ];
